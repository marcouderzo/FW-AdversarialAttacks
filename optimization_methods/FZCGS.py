...
def obj_fun(x_in, S1_batch_idx_in):
    return adv_objective_function() # this should return the value of the adversarial objective function (?)

def estimate_gradient_n(x, MGR, n, d, mu): #mu smoothing parameter, e_j (R^d) basis vector where only j-th element is 1, otherwise 0.
        ...
        for j in range(d):
            ...
            v_estim= v_estim + ((obj_fun(x + mu*e[j, :], S1_batch_idx) - obj_fun(x - mu*e[j, :], S1_batch_idx)) / (2*mu)) * e[j, :] # e to reshape to x0.shape        
        return v_estim


def V(g, u, u_t, gamma, x, OMEGA): # this needs clarification
...
def FZCGS(x_0, n, q, K, L, objective_function, MGR):

    v_k = 10e11
    x_k = x_0  # best_delImgAT
    ...
    for k in range(0, K): #K-1?
        ...
        if np.mod(k, q) == 0: 
            ...
            for j in range(q):
                v_estim = (1/q)* (obj_fun(x_k, S2_batch_idx[j: j+1]) - obj_fun(x_prev, S2_batch_idx[j: j+1]) + v_prev)
